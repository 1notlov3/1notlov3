...
внутренним системам аэропорта: локализую сбой между приложением, базой данных,
...
инструменты автоматизации: на одном из них, **AdminTools**, держится ежедневная
работа первой линии.
...
Утилита техподдержки под Windows. Администратор вводит имя ПК — или ФИО, логин,
телефон, кабинет, что угодно — получает карточку машины и одним кликом выполняет
над ней любое из **26 действий**: от подключения по RDP и установки принтера до
восстановления служб информационной безопасности.

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/admintools-ui-dark.svg">
    <img src="assets/admintools-ui-light.svg" width="100%" alt="Схема интерфейса AdminTools. Строка поиска: имя ПК, ФИО, логин, телефон или кабинет — ищет в Active Directory и в локальной истории. Слева карточка машины: пользователь, телефон, кабинет, ОС, адрес, источник данных. Справа плитка действий: подключиться по RDP, установить принтер, восстановить службы ИБ, диск и очистка, сеанс пользователя, установка ПО, и ещё 20 действий из Config.psd1. Внизу: опрос сети идёт в пуле runspace на 8 потоков, окно остаётся отзывчивым.">
  </picture>
</p>

Написана на **PowerShell 5.1 и WPF** и работает только на штатных компонентах
Windows: ни сторонних библиотек, ни `.exe`, ни RSAT. Для закрытой сети аэропорта
это не придирка, а условие, при котором инструмент вообще можно запустить.

### Что решено внутри

- 🧭 **Поиск не на чем ломаться.** Раньше он жил на `pclist.csv` с сетевого
  диска: диск отвалился — и компьютеры стало не найти вообще. Теперь у поиска
  две ноги: **Active Directory** и локальная накопительная история. Программа
  запоминает каждый просмотренный ПК вместе с пользователем, телефоном и
  кабинетом, и со временем история становится собственной базой, которую никто
  не отберёт;
...
- 🔐 **Безопаснее предыдущей версии.** Пароль администратора больше не уходит в
  командную строку `schtasks.exe`, где его видно в диспетчере задач и в журналах
  аудита, — задача регистрируется через **COM**. Временные XML на общей шаре не
  создаются вовсе;
...
- ⚙️ **Всё настраивается без правки кода.** Пути, серверы, списки служб и сами
  26 действий лежат в `Config.psd1`.

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/admintools-arch-dark.svg">
    <img src="assets/admintools-arch-light.svg" width="100%" alt="Данные: Active Directory для поиска объекта в домене, локальная накопительная история с ПК, пользователем, телефоном и кабинетом, без файловых шар. Выполнение: пул runspace, таймер собирает результат, без DoEvents и Start-Sleep. Безопасность: задача регистрируется через COM, а не через schtasks.exe, пароль не попадает в командную строку и в журналы аудита, временные XML на общий диск не пишутся. Только штатные компоненты Windows — без сторонних библиотек, .exe и RSAT; пути, серверы и все 26 действий описаны в Config.psd1.">
  </picture>
</p>

### Принтеры одним кликом

Выбрали принтер в каталоге сервера печати, нажали «Подключить» — драйвер
ставится **от имени SYSTEM**, перезапускается диспетчер печати, принтер
подключается пользователю, результат проверяется. Раньше это был ручной проход
через оснастку MMC плюс `.bat` на рабочем столе, который у обычного пользователя
чаще всего и не срабатывал.

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="assets/admintools-printers-dark.svg">
    <img src="assets/admintools-printers-light.svg" width="100%" alt="Схема экрана печати AdminTools: фильтр по запросу «терминал 2», каталог сервера печати на 818 принтеров, кнопка «Подключить». Таблица: принтер, размещение, драйвер, статус. Порядок действий: драйвер под SYSTEM, перезапуск спулера, подключение пользователю, проверка результата.">
  </picture>
</p>

> Экраны выше — схемы интерфейса, а не снимки рабочего окна: AdminTools живёт во
> внутренней сети аэропорта, её содержимое наружу не выносится. Имена машин,
> принтеров и людей на схемах вымышленные.

**Стек:** PowerShell 5.1 · WPF · Active Directory · runspace pool · COM ·
`Config.psd1`
...
Те же обращения, что я разбираю руками на второй линии, — только собранные в
датасет. Взял **33 309 обращений** за два года, проверил гипотезы, собрал панель
и написал рекомендации для руководства.
