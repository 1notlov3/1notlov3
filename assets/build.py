...
def hero(C):
    b = [text(64, 100, "IT SUPPORT · 2ND LINE · INTEGRATIONS", font=MONO,
         ...
         text(64, 258, "Разбираю эскалации по внутренним системам", size=20,
              fill=C["eyebrow"]),
         text(64, 286, "компании и довожу проблему до причины.", size=20,
              fill=C["eyebrow"]),
    ...
    b += [line(64, 316, 1136, 316, stroke=C["rule"]),
          text(64, 346, "Вторая линия поддержки · REST API · SQL · JSON · "
                        "XML · Python · PowerShell", size=16.5,
               fill=C["eyebrow"])]
    return b
...
AT_ACTIONS = ["Подключиться по RDP", "Установить принтер",
              "Службы и процессы", "Диск и очистка",
...
def admintools_ui(C):
    ...
    b += [line(88, 522, 1112, 522, stroke=C["stroke"]),
          ...
          rich(88, 582, [{"t": "[14:02:14]", "fill": AMBER},
                         {"t": " Печать: драйвер установлен · спулер "
                               "перезапущен · "},
...
# -------------------------------------------------- admintools: экран 2
PRINTERS = [
    ("PRN-A-0143", "Корпус А · 3 этаж", "Kyocera P3145dn", True),
    ("PRN-A-0187", "Корпус А · 2 этаж", "HP LaserJet M507", True),
    ("PRN-B-0204", "Корпус Б · 1 этаж", "Zebra ZT411", False),
    ("PRN-B-0219", "Корпус Б · 4 этаж", "Kyocera P3145dn", True),
]
PRINT_FLOW = ["установка драйвера", "перезапуск спулера",
...
def admintools_printers(C):
    ...
    b += [rect(64, 96, 1072, 420, rx=14, fill=C["panel"], stroke=C["stroke"]),
          text(88, 128, "AdminTools — печать и драйверы", font=MONO, size=17,
               weight="700", fill=BRIGHT),
          text(1112, 128, "каталог принтеров", font=MONO, size=15, fill=AMBER,
               anchor="end"),
          line(88, 144, 1112, 144, stroke=C["stroke"]),
          rect(88, 164, 680, 44, rx=10, fill=C["inner"], stroke=C["stroke"]),
          text(110, 192, "корпус А", font=MONO, size=18, fill=BRIGHT),
          rect(196, 174, 2, 24, fill=TEAL),
...
ARCH_CARDS = [
    ("ДАННЫЕ", [("Active Directory", "поиск объекта в домене"),
                ...
                ("Работает автономно", "без внешних зависимостей")]),
    ...
    ("ИНТЕРФЕЙС", [("Разметка на сетках", "DPI 125% и 150% без сюрпризов"),
                   ("Иконки — контуры SVG", "а не глифы шрифта"),
...
def admintools_arch(C):
    ...
    b += [line(64, 376, 1136, 376, stroke=C["rule"]),
          ...
          text(64, 432, "Пути, списки служб и все 26 действий описаны в "
...
if __name__ == "__main__":
    write("hero", HERO_W, HERO_H, "Максим Грачев — поддержка интеграций",
          ...
          "Разбираю эскалации по внутренним системам компании и довожу проблему "
          "до причины. Рабочий процесс: 01 эскалация — приём и триаж; "
          "02 локализация — логи, API, SQL; 03 фикс — правка и автоматизация. "
          "Вторая линия поддержки: REST API, SQL, JSON, XML, Python, PowerShell.",
          hero)
    ...
    write("admintools-ui", 1200, 640, "AdminTools — поиск и карточка ПК",
          ...
          "подключиться по RDP, установить принтер, службы и процессы, диск "
          "и очистка, сеанс пользователя, установка ПО, и ещё 20 действий из "
          "Config.psd1. Внизу: опрос сети "
    ...
    write("admintools-printers", 1200, 545, "AdminTools — каталог принтеров",
          "Схема экрана печати AdminTools: фильтр по размещению, каталог "
          "принтеров, кнопка «Подключить». Таблица: принтер, размещение, "
          "драйвер, статус. Порядок действий: установка драйвера, перезапуск "
          "спулера, подключение пользователю, проверка результата.",
          admintools_printers)
    write("admintools-arch", 1200, 460, "AdminTools — как устроено",
          ...
          "работает автономно. Выполнение: пул runspace, таймер собирает "
          "результат, без DoEvents и Start-Sleep. Интерфейс: разметка на "
          "сетках и корректный вид при DPI 125% и 150%, иконки контурами SVG, "
          "тёмная и светлая темы на лету. Только штатные компоненты Windows — "
          "без сторонних библиотек, .exe и RSAT; пути и все 26 действий "
